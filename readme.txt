...
You run the script, answer a few questions and you're done!
###
...
You'll need a raspberry pi (obviously) running raspbian
  	...
  	Raspberry Pi camera enabled in raspi-config “sudo rasps-config”
  	libav-tool installed “sudo apt-get install libav-tools” this is where avconv is
###

Installation:
Copy the timelapseme.sh file to your users home directory and run with 
###
...
**No error checking has been set up for the input parameters**
A warning is displayed stating all previous jpg files will be removed from the target directory
A summary of your selections is then displayed with an estimate of the final timelapse length for confirmation
A new subdirectory of the users home directory will be created if required
...
Constants:
	There are a number of constants that can be changed within the script so it better meets you needs:-	path=$HOME/timelapseme                #Specifies where the images and rendered file will be saved
